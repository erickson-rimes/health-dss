...
header_title = "Facilities heatmap"
...
def heatmap_layout():
    layout = html.Div(
        [
            ...
            html.Div(
                dcc.Graph(id="heatmap_map_display", style={"height": "100%"}),  # Set map height to fill the viewport
...
def right_side():
    return html.Div(
        [
            dcc.Loading(
                id="loading-cards",
                children=[html.Div(id="heatmap_cards_container")],
                # type="circle",
            )
        ]
    )
...
@my_app.callback(
    [
        Output("heatmap_cards_container", "children"),
        Output("heatmap_map_display", "figure"),
    ],
    [
...
def update_facility_content(facilityType, ownership, accreditationStatus, serviceOfferings, municipality, subDistrict, village):
    ...
    # Create a figure with a textual message instead of a map
        ...
        fig = px.density_mapbox(
            ...
            z="ownership",  # Use the 'ownership' column to determine point colors
...
#     [Input('heatmap_map_display', 'hoverData')]
# )
