...
def heatmap_plot_layout():
    layout = html.Div(
        [
            dcc.Store(id="heatmap-case-reports-store"),
            dcc.Loading(
                children=[
...
@my_app.callback(
    Output("heatmap-case-reports-store", "data"),
    [
        Input("analysis_heatmap_plot_date", "start_date"),
        Input("analysis_heatmap_plot_date", "end_date"),
        Input("heatmap_plot_case_type_filter", "value"),
        Input("heatmap_plot_reporting_entity_filter", "value")
    ]
)
...
@my_app.callback(
    ...
    [
        Input("heatmap-case-reports-store", "data"),
        # Input("heatmap-plot-config-dropdown", "value"),
        Input("heatmap-plot-temporal-granularity-dropdown", "value"),
        Input("heatmap_plot_reporting_entity_filter", "value"),
        Input("heatmap_plot_case_type_filter", "value"),
    ],
    [
        State("analysis_heatmap_plot_date", "start_date"),
        State("analysis_heatmap_plot_date", "end_date"),
    ]
)
def update_graph(
    case_reports_data, 
...
def heatmap_plot_content():
    return html.Div(
        [
            ...
            # dcc.Dropdown(
            #     id='heatmap-plot-config-dropdown',
            #     options=[
            ...
            dcc.Dropdown(
                id="heatmap-plot-temporal-granularity-dropdown",
                options=[
            ...
            dcc.Dropdown(
                id="heatmap_plot_case_type_filter",
                options=[
            ...
            dcc.Dropdown(
                id="heatmap_plot_reporting_entity_filter",
                options=[
            ...
            dbc.Button("Download as .CSV", id="heatmap-plot-download-data", color="primary", style={"width": "100%"}),
            dcc.Download(id='heatmap-plot-download-csv')     
        ]
    )

# callback to download the heatmap-case-reports-store as csv
@my_app.callback(
    Output('heatmap-plot-download-csv', 'data'),
    Input("heatmap-plot-download-data", "n_clicks"),
    State("heatmap-case-reports-store", "data"),
        ...
        [
            ...
            html.Div(
                [
                    dbc.Button(
                        "View Code",
                        id="analysis_heatmap_plot_collapse_button",
                        className="mb-3",
                        color="primary",
                        n_clicks=0,
                    ),
                    dbc.Collapse(
                        ...
                        id="analysis_heatmap_plot_collapse",
                        is_open=False,
                    ),
                ]
            ),
            dbc.Button(
                ...
                color="success",
                className="me-1",
                id="analysis_heatmap_plot_download_btn",
            ),
            dcc.Download(id="analysis_heatmap_plot_download"),
        ]
    )


@my_app.callback(
    Output("analysis_heatmap_plot_download", "data"),
    Input("analysis_heatmap_plot_download_btn", "n_clicks"),
    prevent_initial_call=True,
)
def func(n_clicks):
...
@my_app.callback(
    Output("analysis_heatmap_plot_collapse", "is_open"),
    [Input("analysis_heatmap_plot_collapse_button", "n_clicks")],
    [State("analysis_heatmap_plot_collapse", "is_open")],
)
